...
execSync("cd .. && go build -o docs-site/dev-stack ./cmd/dev-stack", {
  stdio: "inherit",
});
...
`;
  ...
  commands.forEach((cmd) => {
    try {
      const cmdHelp = execSync(`./dev-stack ${cmd} --help`, {
        encoding: "utf8",
      });
      markdown += `### ${cmd}\n\n\`\`\`\n${cmdHelp}\`\`\`\n\n`;
    } catch (error) {
...
function generateHomepage() {
  ...
  try {
    ...
    content = content.replace(
      /\[([^\]]+)\]\(LICENSE\)/g,
      "[$1](https://github.com/isaacgarza/dev-stack/blob/main/LICENSE)",
    );
